import React from "react";
import { ShieldCheck, Compass, Target, Award, CheckCircle2 } from "lucide-react";
import { InternalPageLayout } from "../components/InternalPageLayout";
import { getSiteData } from "../data/siteDataService";

interface AboutPageProps {
  openInquiry: () => void;
  onNavigate?: (pageId: string) => void;
}

const iconMap: Record<string, React.FC<{ className?: string }>> = {
  ShieldCheck,
  Compass,
  Target,
  Award,
  CheckCircle2,
};

export const AboutPage: React.FC<AboutPageProps> = ({ openInquiry, onNavigate = () => {} }) => {
  const siteData = getSiteData();
  const school = siteData.schoolInfo;
  const about = siteData.aboutPage;
  const bannerImage = siteData.pageBanners?.about || "https://images.unsplash.com/photo-1523050854058-8df90110c9f1?auto=format&fit=crop&w=1600&q=80";

  return (
    <InternalPageLayout
      title={about?.heading || "About Lotus Global School"}
      category="ABOUT US"
      activePageId="about"
      onNavigate={onNavigate}
      openInquiry={openInquiry}
      bannerImage={bannerImage}
      breadcrumbs={[{ label: "About Us" }]}
    >
      <div className="space-y-8">
        {/* Page Section Heading */}
        <div className="border-b-2 border-[#2F5187] pb-3">
          <span className="text-xs font-bold uppercase tracking-wider text-[#E87737] block">
            {about?.subheading || "Institutional Identity & Background"}
          </span>
          <h2 className="font-display font-bold text-2xl sm:text-3xl text-[#2F5187]">
            {about?.heading || "Overview of Lotus Global School"}
          </h2>
        </div>

        {/* Lead Quote & Image Block */}
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center bg-[#F8FAFC] p-6 rounded border border-slate-200">
          <div className="md:col-span-5 overflow-hidden rounded border border-slate-200 shadow-sm">
            <img
              src={about?.leadImage || "https://images.unsplash.com/photo-1541829070764-84a7d30dd3f3?auto=format&fit=crop&w=800&q=80"}
              alt="Lotus Global School Campus"
              className="w-full h-56 object-cover"
            />
          </div>
          <div className="md:col-span-7 space-y-3">
            <div className="text-xs font-bold uppercase tracking-widest text-[#E87737]">
              Core Educational Creed
            </div>
            <h3 className="font-display font-bold text-lg text-[#2F5187]">
              "{about?.leadQuote || "Learning Should Inspire Rather Than Instruct"}"
            </h3>
            <p className="text-xs sm:text-sm text-slate-700 leading-relaxed font-medium">
              {school.narrative.leadParagraph}
            </p>
          </div>
        </div>

        {/* Narrative Content Body */}
        <div className="space-y-4 text-xs sm:text-sm text-slate-700 leading-relaxed">
          <p>{school.narrative.pedagogy}</p>
          <p>{school.narrative.outlook}</p>
          <div className="p-4 bg-[#FFF5EE] border-l-4 border-[#E87737] rounded-r text-[#2F5187] font-semibold text-xs sm:text-sm">
            "{school.narrative.closing}"
          </div>
        </div>

        {/* Institutional Pillars Grid */}
        <div className="pt-4 space-y-4">
          <h3 className="font-display font-bold text-lg text-[#2F5187] border-b border-slate-200 pb-2">
            Institutional Distinctions
          </h3>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {(about?.distinctions || []).map((distinction, idx) => {
              const IconComponent = iconMap[distinction.icon] || ShieldCheck;
              const colorClasses = [
                "text-[#E87737]",
                "text-[#2F5187]",
                "text-[#E25B88]",
                "text-[#E87737]",
              ];
              const iconColor = colorClasses[idx % colorClasses.length];

              return (
                <div key={idx} className="p-4 rounded border border-slate-200 bg-[#F8FAFC] space-y-2">
                  <div className="flex items-center gap-2">
                    <IconComponent className={`w-4 h-4 ${iconColor}`} />
                    <h4 className="font-bold text-xs sm:text-sm text-[#2F5187] uppercase">
                      {distinction.title}
                    </h4>
                  </div>
                  <p className="text-xs text-slate-600 leading-relaxed">
                    {distinction.description}
                  </p>
                </div>
              );
            })}
          </div>
        </div>
      </div>
    </InternalPageLayout>
  );
};
